...
FakeRange.prototype.read = function (display) {
  ...
  for (var r = 0; r < this.rowCount; r += 1) {
    ...
    for (var c = 0; c < this.columnCount; c += 1) {
      var value = this.row === 1 ? this.sheet.headers[this.column + c - 1] :
...
assert(decisionRows.length === 148 && masterRows.length === 148, 'fixture has 148 persisted candidates');
...
assert(result.afterPendingCount === 2, 'only two manual reviews remain pending');
