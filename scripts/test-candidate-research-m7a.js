...
var excluded = row(masterHeaders.length);
excluded[index(masterHeaders, 'Steam App ID')] = '4026251';
excluded[index(masterHeaders, '游戏名称')] = 'Excluded Game';
excluded[index(masterHeaders, '进入下一步')] = '否';
...
var masterRows = [master, excluded];
spreadsheet = {
...
assert(decision[14] === '未检查' && decision[15] === '人工Social' && decision[16] === '未检查' && decision[17] === '人工关键词' && decision[19] === 'WATCH', 'manual fields untouched');
...
assert(second.created === 0, 'same AppID and cycle are idempotent');

var next = sandbox.enqueueSteamCandidateResearchJobs_(spreadsheet, new Date('2026-08-24T01:00:00Z'));
assert(next.created === 1, 'new research cycle creates one new job');
assert(decision[23] === 'steam-research-4026250-20260824', 'new cycle job id');
assert(decision[15] === '人工Social' && decision[19] === 'WATCH', 'new cycle preserves manual fields');
