...
/**
 * Build rawRecords from existing Historical Raw Observations for a run.
 * Never invents rows and never writes Raw Observations.
 */
function g022RawRecordsFromHistorical_(runId) {
  runId = String(runId || '').trim();
  var rawIndex = g022RawIndex_(g022HistoricalSheets_(SpreadsheetApp.openById(QUALIFICATION_ELIGIBILITY_PRODUCTION_SHEET_ID)).raw, runId);
  return Object.keys(rawIndex).map(function (id) {
    var raw = rawIndex[id] || {};
    return {
      ...
      releaseRaw: raw.releaseRaw,
      releaseStage: raw.releaseStage,
      daysToRelease: raw.daysToRelease
    };
  }).filter(function (rec) { return rec.appId && rec.source; });
}

/**
 * Production repair: finalize Feature/Run Ledger/Canonical for an existing run
 * whose Raw Observations already exist. Does not append Raw Observations.
 * Also marks the business 运行日志 row SUCCESS when historical write succeeds.
 */
function g022FinalizeExistingRunProduction_(runId) {
  runId = String(runId || '').trim();
  if (!runId) throw new Error('runId is required');
  var ss = SpreadsheetApp.openById(QUALIFICATION_ELIGIBILITY_PRODUCTION_SHEET_ID);
  var sheets = g022HistoricalSheets_(ss);
  var rawIndex = g022RawIndex_(sheets.raw, runId);
  var rawCount = Object.keys(rawIndex).length;
  if (!rawCount) {
    return {ok: false, error: 'no_raw_observations', runId: runId, rawCount: 0};
  }

  var featureCount = 0;
  if (sheets.features.getLastRow() >= 2) {
    var featureHeaders = sheets.features.getRange(1, 1, 1, sheets.features.getLastColumn()).getDisplayValues()[0];
    var featureRunCol = featureHeaders.indexOf('Run ID');
    if (featureRunCol >= 0) {
      sheets.features.getRange(2, featureRunCol + 1, sheets.features.getLastRow() - 1, 1)
        .getDisplayValues()
        .forEach(function (row) {
          if (String(row[0] || '').trim() === runId) featureCount += 1;
        });
    }
  }
  var ledgerExists = false;
  if (sheets.runLedger.getLastRow() >= 2) {
    ledgerExists = sheets.runLedger.getRange(2, 1, sheets.runLedger.getLastRow() - 1, 1)
      .getDisplayValues()
      .some(function (row) { return String(row[0] || '').trim() === runId; });
  }

  var active = typeof g010ReadState_ === 'function' ? g010ReadState_() : null;
  var state = active && String(active.runId || '') === runId
    ? Object.assign({}, active)
    : {
      runId: runId,
      runDate: runId.slice(0, 8),
      runStartedAt: 0,
      discoveryComplete: true,
      discoveryAudit: {sources: {}},
      segmentCount: 1,
      ledgerAppended: rawCount,
      enrichmentCursor: 999999,
      controlCursor: 999999,
      phase: 'DONE',
      updatedAt: Date.now()
    };
  state.discoveryComplete = true;
  var metrics = g010ComputeFinalStatsFromMaster_(ss, runId);
  metrics.rawTotal = rawCount;
  metrics.rawPersisted = rawCount;
  var completion = {status: 'SUCCESS', issues: []};

  // Fast path: Raw/Feature/Ledger already consistent — only refresh canonical
  // and mark business audit SUCCESS without rebuilding enrichment context.
  if (featureCount === rawCount && ledgerExists) {
    var canonical = g022RefreshDailyCanonicalRuns_(ss);
    g010UpsertAuditRow_(ss, state, 'SUCCESS', g010FormatSuccessDetail_(state, metrics, completion), metrics);
    if (active && String(active.runId || '') === runId) g010ClearState_();
    ...
    return {
      ...
      ledgerExists: true,
      shortCircuit: true,
      historical: {ok: true, features: {appended: 0, duplicates: featureCount}, ledger: {appended: 0, duplicates: 1}},
      metrics: metrics,
      auditStatus: 'SUCCESS',
      canonicalRows: canonical.rows,
      rawObservationsRewritten: false
    };
  }

  var rawRecords = Object.keys(rawIndex).map(function (id) {
    var raw = rawIndex[id] || {};
    return {
      appId: String(raw.appId || '').trim(),
      name: raw.name || '',
  ...
  var context = g010BuildRunContext_(ss, state, startedAt, ss.getSpreadsheetTimeZone());
  // Ground truth is the already-persisted Raw Observations; never rewrite them.
  context.rawRecords = rawRecords;
  var historical = g022FinalizeHistoricalRun_(
    ss, state, context, metrics, completion, new Date(), {scheduledDaily: true}
  );
  if (!historical.ok) {
    return {
      ok: false,
      ...
      error: historical.error || 'historical_finalize_failed'
    };
  }
  g010UpsertAuditRow_(ss, state, 'SUCCESS', g010FormatSuccessDetail_(state, metrics, completion), metrics);
  if (active && String(active.runId || '') === runId) g010ClearState_();
